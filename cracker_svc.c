...
static void cracker_1(struct svc_req *rqstp, SVCXPRT *transp);
...
main( int argc, char* argv[] )
{
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, cracker, 0, cracker_1, proto)) {
			_msgout("unable to register (cracker, 0, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, cracker, 0, cracker_1, proto)) {
			_msgout("unable to register (cracker, 0, tcp).");
			exit(1);
		}
	}
