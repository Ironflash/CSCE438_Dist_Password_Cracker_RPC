...
lsp_client* lsp_client_create(const char* dest, int port){
    ...
    if(network_send_connection_request(client->connection)){
        fprintf(stderr,"trigger send_message_to_client\n");
        lsp_client *arg1 = client;
        networkMessage *arg2 = getGlobalMessage();
        send_message_to_client(arg1, arg2);
...
void* ClientWriteThread(void *params){
    ...
    while(true){
        ...
        if(nextToSend > lastSent){
            ...
            if(client->connection->outbox.size() > 0) {
                networkMessage* msg = (networkMessage*) network_send_message(client->connection,client->connection->outbox.front());
                fprintf(stderr,"trigger send_message_to_client\n");
                lsp_client *arg1 = client;
                networkMessage *arg2 = getGlobalMessage();
                send_message_to_client(arg1, arg2);
